 ...
 * works across different applications servers.
 *
 * <p>It has come out of many months of frustrating use of multiple application servers at
 * Atlassian, please don't change things unless you're sure they're not going to break in one server
 * or another!
 */
public class ClassLoaderUtil {

  /**
   * Load all resources with a given name, potentially aggregating all results from the searched
   * classloaders. If no results are found, the resource name is prepended by '/' and tried again.
   *
   * <p>This method will try to load the resources using the following methods (in order):
   *
   * <ul>
  ...
  public static Iterator<URL> getResources(
    ...
    if (!iterator.hasNext() && (resourceName.length() == 0 || resourceName.charAt(0) != '/')) {
   ...
   *
   * <p>This method will try to load the resource using the following methods (in order):
   *
   * <ul>
   *   <li>From Thread.currentThread().getContextClassLoader()
   *   <li>From ClassLoaderUtil.class.getClassLoader()
   ...
   *
   * <p>The algorithm used to find the resource is given in getResource()
   *
  ...
  static class AggregateIterator<E> implements Iterator<E> {
    ...
    public void addEnumeration(Enumeration<E> e) {
